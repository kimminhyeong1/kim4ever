	...
	<div id="content">
		...
		<c:forEach var="kv" items="${klist}">
			<tr>
				<td>${kv.bikeCode}</td>
				<td>${kv.bikeType}</td>
				<td>${kv.bikeLocation} 대여소</td>
			<td>
				<c:choose>
					<c:when test="${kv.bikeState eq 'Y'}">대여 가능</c:when>
					<c:when test="${kv.bikeState eq 'N'}">대여 중</c:when>
					<c:when test="${kv.bikeState eq 'E'}">고장</c:when>
					<c:otherwise>${kv.bikeState}</c:otherwise>
				</c:choose>
			</td>
		</tr>
	</c:forEach>
