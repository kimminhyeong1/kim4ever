...
//AES알고리즘으로 암호화 복호화 
    ...
    public static String encrypt(String plainText) throws Exception {//암호화
        ...
        String encryptedText = Base64.getEncoder().encodeToString(encryptedBytes); //암호화된 바이트 배열을 Base64로 인코딩하여 문자열로 반환
        encryptedText = encryptedText.replace("/", "_"); // '/'를 '_'로 대체
    ...
    public static String decrypt(String encryptedText) throws Exception {//복호화
    	...
    	encryptedText = encryptedText.replace("_", "/"); // '_'를 '/'로 복원
